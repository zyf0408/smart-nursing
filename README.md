...
| 1.0.4 | 2026-07-21 | 修复考试判分系列问题（答案格式、判断题归一化、多选题脏数据、结果页字段不匹配） |

### 1.0.4 详细变更

**考试判分系列修复**：本次集中修复了考试结果0分及判分异常的多个问题，涉及 5 个独立 bug：

1. **答案格式不匹配导致0分**：移动端交卷提交数组格式 `[{questionId, answer}]`，后端期望 Map 格式 `{"questionId":"answer"}`，Jackson 反序列化失败返回空 Map，全部判错0分。修复为前端提交 Map 格式。

2. **判断题答案未归一化**：数据库 `answer='false'`，前端提交 `'F'`，`'F'.equalsIgnoreCase('false')` 因长度不同返回 false。修复为后端新增 `normalizeJudgeAnswer` 方法，统一将 true/false/T/F/正确/错误/对/错 转为 T/F 后比较。

3. **多选题脏数据判错**：管理端编辑页用 `split(',')` 拆分 `"ABCD"` 得到 `["ABCD"]`，重新勾选保存后 answer 变成 `"ABCD,A,B,C,D"`（畸形数据），导致判错。修复方案：后端 `sortAnswer` 用 TreeSet 去重只保留字母；前端用正则提取字母、`join('')` 无分隔符拼接；数据库脏数据已清理。

4. **结果页字段名不匹配**：后端返回 `answerDetails`，前端读 `answers`/`details`，字段名不匹配导致统计全0。同时 `questionType` 数字与前端期望的字符串不匹配。修复为前端读取 `answerDetails` 并做题型映射、选项构建。

5. **管理端试题选择弹窗不加载数据**：`ExamEdit.vue` 点击"添加试题"只打开弹窗不调用 `loadQuestions()`。修复为 el-dialog 添加 `@open="loadQuestions"`。
