  ...
  <view class="learn-page">
    ...
    >
      ...
      <view v-if="contentList.length > 0" class="content-list">
        ...
        >
          <!-- 视频类型 -->
          <view v-if="item.type === 'video'" class="card-video">
            <view class="video-cover">
              <image v-if="item.coverUrl" :src="item.coverUrl" class="cover-img" mode="aspectFill" />
            ...
            <view class="card-body">
              ...
              <view class="card-meta">
                ...
                <text class="meta-text">{{ item.categoryName || '未分类' }}</text>
          ...
          <!-- 文章类型 -->
          <view v-else-if="item.type === 'article'" class="card-article">
            ...
            <image v-if="item.coverUrl" :src="item.coverUrl" class="article-thumb" mode="aspectFill" />
          </view>

          <!-- PPT/课件类型 -->
          <view v-else-if="item.type === 'ppt'" class="card-ppt">
            ...
            <view class="card-body">
              ...
              <view class="card-meta">
                ...
                <text class="meta-text">{{ item.categoryName || '未分类' }}</text>
                <text class="meta-text">{{ item.pageCount || 0 }}页</text>
                <text class="meta-text meta-time">{{ formatTime(item.createTime) }}</text>
              </view>
            </view>
          </view>
          ...
          <view v-else class="card-default">
            <view class="card-body">
              <text class="card-title text-ellipsis-2">{{ item.title }}</text>
              <view class="card-meta">
                <text class="meta-tag">{{ item.type || '内容' }}</text>
                <text class="meta-text">{{ item.categoryName || '未分类' }}</text>
              </view>
            </view>
          </view>
...
const categories = ref([
  ...
  { label: '急救护理', value: 'emergency' },
  { label: '护理管理', value: 'management' },
  { label: '感染控制', value: 'infection' },
  { label: '药理知识', value: 'pharmacology' },
...
const loadList = async (reset = false) => {
  ...
  try {
    ...
    if (contentList.value.length >= total || list.length < pageSize) {
      noMore.value = true
    } else {
      page.value++
    }
...
const goDetail = (item) => {
  let url = ''
  if (item.type === 'video') {
    url = `/pages/learn/video-detail?id=${item.id}`
  } else if (item.type === 'ppt') {
    url = `/pages/learn/ppt-detail?id=${item.id}`
  } else {
    url = `/pages/learn/article-detail?id=${item.id}`
  }
  uni.navigateTo({ url })
}
