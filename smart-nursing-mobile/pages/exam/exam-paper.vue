...
onLoad((options) => {
  ...
  examTitle.value = options.title || '考试答题'
  ...
  if (paperDataStr) {
    try {
      ...
      examTitle.value = paperInfo.examTitle || examTitle.value
      // 解析试卷题目
      const paperData = paperInfo.paperData
      const questionList = paperData.questions || paperData.list || paperData || []
      // 打乱题目顺序（如果后端支持 shuffle 或前端需要打乱）
      questions.value = shuffleQuestions(questionList)
      // 初始化答案对象
