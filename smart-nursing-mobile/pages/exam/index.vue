      ...
      <view v-if="examList.length > 0" class="exam-list">
        ...
        >
          <view class="exam-card-header">
            <view class="exam-icon-wrapper" :class="getStatusClass(item.status)">
              <text class="exam-icon">📝</text>
            </view>
            <view class="exam-info">
              <text class="exam-title text-ellipsis">{{ item.title }}</text>
              <view class="exam-tags">
                <text class="exam-tag" :class="getStatusClass(item.status)">{{ getStatusText(item.status) }}</text>
                <text v-if="item.categoryName" class="exam-tag tag-normal">{{ item.categoryName }}</text>
              </view>
            </view>
          </view>
          ...
          <view class="exam-card-footer">
            <view class="exam-time">
              <text v-if="item.status === 'available'" class="time-text time-available">
                截止: {{ formatExamTime(item.endTime) }}
              </text>
              <text v-else-if="item.status === 'ongoing'" class="time-text time-ongoing">
                进行中 · 剩余 {{ item.remainingTime || '未知' }}
              </text>
              <text v-else-if="item.status === 'completed'" class="time-text time-completed">
                已完成 · 得分 {{ item.score || 0 }}
              </text>
              <text v-else-if="item.status === 'expired'" class="time-text time-expired">
                已截止
              </text>
              <text v-else class="time-text">{{ getStatusText(item.status) }}</text>
            </view>
            <view class="exam-action">
              <text v-if="item.status === 'available'" class="action-btn btn-start">开始考试</text>
              <text v-else-if="item.status === 'ongoing'" class="action-btn btn-continue">继续答题</text>
              <text v-else-if="item.status === 'completed'" class="action-btn btn-result">查看结果</text>
              <text v-else class="action-btn btn-disabled">不可用</text>
            </view>
          </view>
        </view>
      </view>
...
const loadList = async () => {
  ...
  try {
    ...
    const res = await exam.getExamList(params)
    examList.value = res.list || res.records || res || []
...
const goExamDetail = (item) => {
  if (item.status === 'completed') {
    // 已完成的考试直接跳转结果页
    uni.navigateTo({
      url: `/pages/exam/exam-result?recordId=${item.recordId || item.id}`
    })
  } else {
    uni.navigateTo({
      url: `/pages/exam/exam-detail?id=${item.id}`
    })
  }
}
